    ...
    var body: some View {
        ZStack {
            ...
            if let background = Brand.image("background") {
                // Bound explicitly to the actual screen geometry — `.aspectRatio(.fill)`
                // alone (without a frame) lets the image's native aspect ratio propose
                // an oversized width, which can get baked into the window's layout on
                // the very first pass and persist after the splash is dismissed.
